...
import com.mdy.basis.cache.redis.RedisCacheProvider;
...
public class RedisCacheChannel extends BinaryJedisPubSub implements CacheExpiredListener, CacheChannel {
	...
	private String name;
	private static String channel = BasisCache.getConfig().getProperty("redis.channel_name");
	...
	public CacheObject get(String region, Object key) {
		...
		if (region != null && key != null) {
			try {
				...
				e.printStackTrace();
				log.warn("get[regin="+region+",key="+key+"] error | e="+e);
			}
	...
	public void set(String region, Object key, Object value) {
		if (region != null && key != null) {
			try {
				...
				e.printStackTrace();
				log.warn("set[regin="+region+",key="+key+"] error | e="+e);
			}
	 ...
	 *            : Cache value
	 */
	public void set(String region, Object key, Object value,int expireTime) {
		if (region != null && key != null) {
			try {
				if (value == null)
					evict(region, key);
					...
					// 4. L1 和 L2 都有
					_sendEvictCmd(region, key);// 清除原有的一级缓存的内容
					CacheManager.set(LEVEL_1, region, key, value,expireTime);
					CacheManager.set(LEVEL_2, region, key, value);
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	public void onMessage(byte[] channel, byte[] message) {
		...
		try {
			...
			case Command.OPT_DELETE_KEY:
				log.info("cache | 收到订阅消息:删除 | region={},key={}",cmd.getRegion(),cmd.getKey());
				onDeleteCacheKey(cmd.getRegion(), cmd.getKey());
				break;
			case Command.OPT_CLEAR_KEY:
				log.info("cache | 收到订阅消息:清空 | region={}",cmd.getRegion());
